@echo off
REM Build script for Alteryx OpenYXDB write benchmark using CMake + MSVC
...
echo.
echo Building write benchmark executable...
cl.exe /nologo /EHsc /O2 /std:c++17 /DUNICODE /D_UNICODE /DSRCLIB_REPLACEMENT /DBUILDING_OPEN_ALTERYX /D_CRT_SECURE_NO_WARNINGS /wd4100 /wd4267 /wd4244 /wd4458 ^
    /I"%SRCDIR%\include" ^
    /Fe"%OUTDIR%alteryx_openyxdb_write_benchmark.exe" ^
    "%OUTDIR%alteryx_write_benchmark.cpp" ^
    /link /MACHINE:X64 /LIBPATH:"%BUILDDIR%" Open_AlteryxYXDB.lib 2>&1

if errorlevel 1 (
    echo ERROR: Write benchmark compilation failed
    exit /b 1
)

echo.
echo Build successful: %OUTDIR%alteryx_openyxdb_write_benchmark.exe
