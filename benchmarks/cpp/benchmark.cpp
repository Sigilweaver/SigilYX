// Open_AlteryxYXDB Read Benchmark
//
// Benchmarks the Open_AlteryxYXDB (C++) library's read performance on YXDB files.
// Outputs JSON results to stdout for consumption by the cross-language
// benchmark orchestrator.
//
// Usage:
//     benchmark.exe <path_to_yxdb> [runs]
//
// Methodology:
...
//   - Each run opens the file, iterates all records via ReadRecord(),
//     extracts all field values via GetAsDouble/GetAsWString/GetAsBlob,
//     and finishes when ReadRecord() returns NULL.
...
// Read all records and extract all field values
BenchmarkResult read_all_records(const wchar_t* file_path) {
    ...
    while (const SRC::RecordData* pRec = file.ReadRecord()) {
        for (unsigned i = 0; i < num_fields; ++i) {
            ...
            // Extract value by type, matching what other benchmarks do
            switch (pField->m_ft) {
                case SRC::E_FT_Bool:
...
int main(int argc, char* argv[]) {
    ...
    // Output JSON
    printf("{\n");
    printf("  \"library\": \"open-alteryx-yxdb\",\n");
    printf("  \"language\": \"C++\",\n");
    printf("  \"file\": \"%s\",\n", fname.c_str());
    printf("  \"rows\": %lld,\n", info.rows);
    printf("  \"cols\": %d,\n", info.cols);
