// Alteryx OpenYXDB Dump Tool
//
// Reads a YXDB file using the official Alteryx OpenYXDB (C++) library
// and dumps all field values as tab-separated text to stdout.
// Null values are represented as "\N".
//
// This is the official Alteryx implementation counterpart to
// open_yxdb_dump.exe (NedHarding fork). Used by the cross-implementation
// round-trip test to verify that SigilYX produces files that the official
// Alteryx library can read correctly, and vice versa.
//
// Usage:
//     alteryx_openyxdb_dump.exe <path_to_yxdb>
//
// Output format:
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        SRC::WString wpath(file_path.c_str());
        file.Open(wpath);
